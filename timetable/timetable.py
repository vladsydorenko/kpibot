...
# from miscellaneous.utils import reply, transliterate, get_current_week
# from miscellaneous.arrays import commands, types, days

# from timetable.models import Chat, Group
...
#                     self.teachers_name += token if not self.teachers_name else " " + token
#                 else:
...
#             reply(self.chat_id, msg=self.responses['wrong_parameter'])
#         elif (command != "/setteacher" and command != "/teacher") and (self.teachers_name or self.teacher_id):
#             reply(self.chat_id, msg=self.responses['wrong_parameter'])
#         elif (command == "/setteacher" or command == "/teacher") and not self.teacher_id and not self.teachers_name:
...
#     def setteacher(self):
#         if self.teachers_name:
#             self.answer_teachers_name()
#         else:
#             self.setteacher_id()

#     def answer_teachers_name(self, is_teachertt=False):
#         raw_response = requests.get(settings.TIMETABLE_URL + "teachers/?search=%s" % self.teachers_name)
...
#             else:
#                 result = self.responses['setteacher_filter'].format(self.teachers_name)
...
#     def teacher(self):
#         if self.teachers_name:
#             self.answer_teachers_name(is_teachertt=True)
#         else:
#             self.print_teacher_tt()
...
#             else:
#                 command = message.split()[0].split('@')[0]
#                 if self.print_teacher or (self.group_id != 0 and command != "/setgroup"):
